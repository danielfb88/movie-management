...
describe('User update integration tests', () => {
  ...
  describe('PUT /v1/user/:id', () => {
    ...
    test('Should return NOT_FOUND error when not sent required id param', async done => {
      const res = await request(app).put(`${endpoint}/`).send({}).set('Authorization', token.bearerToken)
